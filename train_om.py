        ...
        for step, train_batch in enumerate(train_loader):
            ...
            if args.model == 'bert':
                ...
                ###
                    ...
                    i += 1
                    input_ids_temp = train_batch['input_ids']
                    input_mask_temp = train_batch['input_mask']
                    segment_ids_temp = train_batch['segment_ids']

                    if 1 not in train_batch['label']:
                        
                        print('yes')
                        input_ids_temp = input_ids_temp.cpu().detach().numpy()
                        input_mask_temp = input_mask_temp.cpu().detach().numpy()
                        segment_ids_temp = segment_ids_temp.cpu().detach().numpy()
                        print(type(input_ids_temp))
                        input_ids_temp = torch.tensor(input_ids_temp, requires_grad=False)
                        input_mask_temp = torch.tensor(input_mask_temp, requires_grad=False)
                        segment_ids_temp = torch.tensor(segment_ids_temp, requires_grad=False)

                    input_ids_list.append(input_ids_temp)
                    input_mask_list.append(input_mask_temp)
                    segment_ids_list.append(segment_ids_temp)
                    label_list.append(train_batch['label'])

                    if i == 10:

                        input_ids_cat = torch.cat(input_ids_list, dim=0)
                        input_mask_cat = torch.cat(input_mask_list, dim=0)
                        segment_ids_cat = torch.cat(segment_ids_list, dim=0)
                        label_cat = torch.cat(label_list, dim=0)
                        print(input_ids_cat.size())
                        print(input_mask_cat.size())
                        print(segment_ids_cat.size())
                        print(label_cat.size())

                        with sync_context():
                            batch_score, logits = model(input_ids_cat.to(device), input_mask_cat.to(device), segment_ids_cat.to(device))
